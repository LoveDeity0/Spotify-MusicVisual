...
for (var i = 0; i < items.length; i++) {
    ...
    // 设置背景图片
    frontBox.style.backgroundImage = 'url(/static/cover/' + (i + 1).toString().padStart(2, '0') + '.png)';
    leftBox.style.backgroundImage = 'url(/static/cover/' + (i + 1).toString().padStart(2, '0') + '.png)';
    rightBox.style.backgroundImage = 'url(/static/cover/' + (i + 1).toString().padStart(2, '0') + '.png)';
}
(function () {
    "use strict";
    ...
    // 定义变量
    var width, height, totalWidth, margin = 10,
        currIndex = 0,
    ...
    function move(index) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (i == (index - 1)) {
                ...
                box.style.transform = "perspective(500px) rotateY(" + (i < (index - 1) ? 40 : -40) + "deg)";
            }
        }
        // 移动slider
        let offset = -400;  // 可手动调节，比如试试 -100 / +100
        slider.style.transform = "translate3d(" +
            ((index * -width) + (width / 2) + window.innerWidth / 2 + offset) + "px, 0, 0)";
        // 设置body背景图片
